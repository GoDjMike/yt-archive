...
def parse_targets_file(targets_path):
    ...
    Recognized per-entry fields (one URL line followed by indented fields):
     ...
     - series: <name>
     - filepath: <path>
     - complete: true|true/false (required)
    """
...
def download_audio(url, series, files_dir, audio_format=AUDIO_FORMAT):
    ...
    try:
        ...
        if not series:
            return False, (
                "No series specified for URL and cannot infer one. "
                "Add `series: <name>` to the entry in targets.md."
            )
        ...
        if os.path.exists(expected):
            ...
            for candidate in os.listdir(full_output_dir):
                if safe_title in candidate and candidate.endswith((".opus", ".mp3", ".m4a", ".aac", ".wav")):
        ...
        if duration:
            ...
            if downloaded_duration:
                ...
                print(f" Downloaded: {downloaded_duration:.0f}s ({downloaded_duration / 3600:.1f}h)")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=f"Audio format to request from yt-dlp (default: {AUDIO_FORMAT})",
    )
